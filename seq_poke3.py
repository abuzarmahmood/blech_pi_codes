...
def seq_poke3(outputs = [31, 33, 35, 37], intaninputs = [24, 26, 19, 21], opentimes = [0.01, 0.01, 0.01, 0.01], iti = 10, trials = 360):
	...
	poke_latencies = np.zeros((trials,2)) #make array for storing poke latencies
		...
		while time.time() < start_time + iti:
			while time.time() < start_time + 1 and sensor == 0:
				...
				if GPIO.input(poke) == 0 and sensor == 0:
					...
					poke_latencies[trial,0] = current_time
					poke_latencies[trial,1] = outputs[i]
					GPIO.output(light, 0)
